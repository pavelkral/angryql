...
public:
    ...
    void UpdatePointsForAnim(bool isMeasuredFrame, const glm::vec2 movementDir,const float aimTheta, float time);
...
private:
    ...
    void processNode(int *meshesProcessed, bool isMeasuredFrame, aiNode *node,
                     const aiScene *scene, int depth,
                     std::unordered_map<std::string, aiMatrix4x4> &);
    PlayerMesh processMesh(bool isMeasuredFrame, aiMesh *mesh,
                           const aiScene *scene,
                           std::unordered_map<std::string, aiMatrix4x4> &);
    std::vector<Vertex> getMeshVertices(
        const bool isMeasuredFrame, aiMesh *mesh, const aiScene *scene,
